...
function buildMenu() {
  ...
  el.id = 'gameMenu';
  el.innerHTML = '<div class="menuShell"></div>';
...
(async () => {
  try {
    ...
    if (bro.controllerGroup) { bro.controllerGroup.position.set(0, 1.845, 0.58); bro.controllerGroup.rotation.set(0, 0, 0); bro.controllerGroup.visible = false; }
    ...
    THREE.Raycaster.prototype.intersectObject = function(object, recursive = true, optionalTarget = []) {
      if (!recursive || !object?.traverseVisible || (object.children?.length || 0) < 10) return nativeIntersectObject.call(this, object, recursive, optionalTarget);
      ...
      const now = performance.now(), ro = this.ray.origin, rd = this.ray.direction;
      ...
      if (moved || now - c.lastAt > 95) {
        ...
        const hits = []; for (const candidate of c.candidates) nativeIntersectObject.call(this, candidate, false, hits);
        hits.sort((a,b)=>a.distance-b.distance); c.hits = hits.slice(0, 4);
      }
      optionalTarget.push(...c.hits); return optionalTarget;
    };

    let bypassActionClick = false, firing = false, fireStartedAt = 0;
    function legacyActionToggle() { bypassActionClick = true; actionBtn.click(); bypassActionClick = false; }
    actionBtn.addEventListener('click', e => { if (!bypassActionClick) { e.preventDefault(); e.stopImmediatePropagation(); } }, true);
    function startFire(e) { if (firing) return; e?.preventDefault?.(); firing = true; fireStartedAt = performance.now(); actionBtn.classList.add('active'); actionBtn.textContent = 'ZAP'; legacyActionToggle(); }
    function stopFire(e) { e?.preventDefault?.(); if (!firing) return; const wait = Math.max(0, 115 - (performance.now() - fireStartedAt)); setTimeout(() => { if (!firing) return; firing = false; legacyActionToggle(); actionBtn.classList.remove('active'); actionBtn.textContent = 'ZAP'; }, wait); }
    ...
    window.addEventListener('keydown', e => { if (e.key.toLowerCase() !== 'f') return; e.preventDefault(); e.stopImmediatePropagation(); if (!e.repeat) startFire(e); }, true);
    window.addEventListener('keyup', e => { if (e.key.toLowerCase() !== 'f') return; e.preventDefault(); e.stopImmediatePropagation(); stopFire(e); }, true);

    let portalArmed = true, autoActivatePending = false;
    function monitorPortal() {
      const p = bro.root?.position;
      if (p) {
        const d = Math.hypot(p.x, p.z), loadReady = !dropBtn.disabled && /LOAD HERO/i.test(dropBtn.textContent || '');
        if (!document.body.classList.contains('menuOpen') && portalArmed && loadReady && d < 1.42) { portalArmed = false; autoActivatePending = true; dropBtn.click(); }
        if (autoActivatePending && !activateBtn.disabled && /ACTIVATE PORTAL/i.test(activateBtn.textContent || '')) { autoActivatePending = false; activateBtn.click(); }
    ...
    function showMenu() { stopFire(); document.body.classList.add('menuOpen'); menu.classList.add('show'); }
    function hideMenu() { document.body.classList.remove('menuOpen'); menu.classList.remove('show'); }
    window.__showGameMenu = showMenu; window.__hideGameMenu = hideMenu;

    let bypassReset = false;
    resetBtn.addEventListener('click', e => {
      if (bypassReset) return;
      e.preventDefault(); e.stopImmediatePropagation(); bypassReset = true; resetBtn.click(); bypassReset = false; showMenu();
    }, true);

    function updateSelectedLabel() {
      if (characterStatus && !/ENTERING|PORTAL READY|CONVERTING|TRANSMITTED/.test(characterStatus.textContent || '')) {
        const motion = bro.motion?.toUpperCase?.() || 'IDLE'; characterStatus.textContent = `${selectedLabel} · ${motion}`;
      }
      actionBtn.textContent = 'ZAP'; if (bro.controllerGroup) bro.controllerGroup.visible = false; requestAnimationFrame(updateSelectedLabel);
    }

    applyHero(localStorage.getItem('gamerBroHero') === 'gb1' ? 'gb1' : 'gb2');
    monitorPortal(); updateSelectedLabel();
  } catch (err) { console.error('v0.8.5 overlay failed:', err); }
})();
