  ...
  function PlayerBlob(parts,options){
    ...
    try{
      if(options?.type==='text/javascript'&&parts?.length===1&&typeof parts[0]==='string'&&parts[0].includes('PORTAL STATE MACHINE — HERO LOAD')){
        ...
        // Expose renderer/scene/camera only so the cinematic can show the ACTUAL map and
        // capture the ACTUAL current hero. Gameplay variables remain private.
        ...
        rep('const camera=new THREE.PerspectiveCamera(36,1,.15,125);','const camera=new THREE.PerspectiveCamera(36,1,.15,125);window.__gamerCamera=camera;','camera');
        ...
        if(pointerPattern.test(source)){
          ...
          changed.push('Crystal drag look');
        }else console.warn('[v0.8.9 bootstrap miss] Crystal drag look');
