...
function completeGame(){if(state.complete)return;state.complete=true;hero.enabled=false;pulse(new pc.Vec3(0,3,-4),MAT.cyan,10,.42);setTimeout(()=>{const sec=((performance.now()-state.start)/1000).toFixed(1);resultsStats.textContent=`${state.leaves} Maple Leaves · ${state.kills} enemies defeated · ${sec}s`;results.hidden=false;requestAnimationFrame(()=>results.classList.add('show'));},430);}
