...
def run(label,mobile=False):
    ...
    try:
        ...
        while time.time()<end:
            ...
            if fatal: raise RuntimeError(d.execute_script("return document.getElementById('error')?.textContent"))
            if ready: break
            time.sleep(.25)
        if not ready: raise RuntimeError('Pass16 did not become ready')
