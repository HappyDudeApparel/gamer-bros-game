...
import {createGamerBro} from '../playground-v2/gamer-bro.js?v=pass9b';
...
const mats={
 grass:new THREE.MeshStandardMaterial({color:0x69a449,roughness:.95}),
 path:new THREE.MeshStandardMaterial({color:0xc7aa79,roughness:.98}),
 stone:new THREE.MeshStandardMaterial({color:0x777a72,roughness:.9}),stone2:new THREE.MeshStandardMaterial({color:0x97998d,roughness:.9}),
 wood:new THREE.MeshStandardMaterial({color:0x7a4b2d,roughness:.9}),water:new THREE.MeshPhysicalMaterial({color:0x2f9abe,roughness:.15,transparent:true,opacity:.82}),
 roof:new THREE.MeshStandardMaterial({color:0x8f4d3c,roughness:.86}),plaster:new THREE.MeshStandardMaterial({color:0xe4d5ad,roughness:.95}),leaf:new THREE.MeshStandardMaterial({color:0x4f8c47,roughness:1})
};
function elev(z){if(z>=-55)return 0;if(z<=-90)return 5;let t=(-55-z)/35;t=t*t*(3-2*t);return t*5}
function terrain(){
 const sx=18,sz=100,w=38,zMax=40,zMin=-151,pos=[],uv=[],idx=[];
 for(let iz=0;iz<=sz;iz++){const z=THREE.MathUtils.lerp(zMax,zMin,iz/sz),y=elev(z);for(let ix=0;ix<=sx;ix++){const x=THREE.MathUtils.lerp(-w/2,w/2,ix/sx);pos.push(x,y,z);uv.push(ix/sx,iz/sz)}}
 ...
 const m=new THREE.Mesh(g,mats.grass);m.name='continuous-world1-terrain';m.receiveShadow=true;m.castShadow=false;world.add(m);walkMeshes.push(m);return m;
}
...
function buildWorld(){
 progress('Building one continuous World 1 landmass…',18);terrain();
 // Paths are visual only: terrain is the sole movement collision surface.
 for(const [z,d] of [[18,35],[-12,25],[-51,10],[-73,31],[-108,29],[-133,20]])box('path',0,elev(z)+.035,z,5,.07,d,mats.path);
 for(const [x,z,s] of [[-11,30,1.05],[11,28,.9],[-12,16,.85],[12,9,1],[-10,-28,.75],[11,-30,.8],[-13,-94,.85],[13,-100,.85],[-12,-126,.85],[12,-139,.9]])tree(x,z,s);
 for(let i=0;i<6;i++){box('fence',-4.8,.55,29-i*4,.18,1.1,3.1,mats.wood);box('fence',4.8,.55,29-i*4,.18,1.1,3.1,mats.wood)}
 house(-9,-5,.9,.08);house(9,-7,1,-.12);house(-10,-15,.82,-.05);house(10,-17,.92,.12);house(-8,-23,.74,.06);house(8,-24,.78,-.08);
 box('village-square',0,.05,-15,11,.10,7,mats.path);cyl('well',-4,.65,-15,1.1,1.1,mats.stone);cyl('well-cap',-4,1.25,-15,1.35,.18,mats.stone2);
 // River exists visually on both sides of a broad bridge corridor; land remains physically continuous underneath.
 box('river-left',-12,-.12,-40,14,.10,11,mats.water);box('river-right',12,-.12,-40,14,.10,11,mats.water);
 for(let z=-44.5;z<=-35.5;z+=1.05)box('bridge-plank',0,.10,z,6,.18,.86,mats.wood);
 for(const x of [-3.25,3.25]){for(let z=-44;z<=-36;z+=2)box('bridge-post',x,.9,z,.18,1.8,.18,mats.wood);box('bridge-rail',x,1.35,-40,.16,.18,9,mats.wood)}
 arch(0,-103,1.05);arch(-8,-110,.85,.08);arch(8,-114,.75,-.06);
 box('ruin-platform-a',-6,elev(-109)+.5,-109,4.5,1,4.5,mats.stone);box('ruin-platform-b',6,elev(-115)+.75,-115,4.5,1.5,4.5,mats.stone);box('ruin-platform-c',0,elev(-121)+1,-121,5.5,2,4.5,mats.stone);
 box('shrine-court',0,elev(-133)+.08,-133,17,.16,15,mats.stone2);box('shrine-step',0,elev(-138)+.22,-138,9,.44,5,mats.stone);
 for(const x of [-8,8]){cyl('shrine-column',x,elev(-132)+2,-132,.55,4,mats.stone);cyl('shrine-cap',x,elev(-132)+4.05,-132,.8,.2,mats.stone2)}arch(0,-144,1.25);
 world.updateMatrixWorld(true);worldBounds=new THREE.Box3().setFromObject(world);
 window.__worldSource='pass9b-continuous-terrain';window.__worldRoute='meadow-village-river-bridge-hillside-ruins-portal-shrine';window.__enemyCount=0;window.__continuousLand=true;window.__movementCollisionMeshes=1;
 return{spawn:{x:0,y:.02,z:31},tube:{x:0,y:elev(-138)+.44,z:-138}}
}
const heightCache=new Map();function hkey(x,z){return`${Math.round(x*10)},${Math.round(z*10)}`}
function groundAt(x,z,cache=true){const key=hkey(x,z);if(cache&&heightCache.has(key))return heightCache.get(key);const top=worldBounds.max.y+18;ray.set(new THREE.Vector3(x,top,z),down);ray.far=(top-worldBounds.min.y)+30;const hit=ray.intersectObjects(walkMeshes,false)[0],y=hit?hit.point.y:null;if(cache)heightCache.set(key,y);return y}

let bro,heroRoot,tube,sites;
const player={x:0,y:0,z:0,yaw:Math.PI,vy:0,speed:0,grounded:true,jumpCount:0,holdJumpTimer:0};
...
const STEP_HEIGHT=.70,GRAVITY=8.25,JUMP_V=5.05,DOUBLE_JUMP_V=4.65,HOLD_BOOST_V=3.95,HOLD_BOOST_INTERVAL=.22;
...
function createTube(){progress('Installing canonical Crystal Library v2 tube…',67);tube=createCrystalLibraryTubeV2({renderer,scene,camera,bro,heroRoot,mobile,onComplete:()=>{objective.textContent='WORLD 1 ROUTE COMPLETE · PASS 9B';retry.classList.add('show');toast('ROUTE COMPLETE',2200)}});tube.root.position.set(sites.tube.x,sites.tube.y-.42,sites.tube.z);window.__tubeComponent='crystal-library-v2-tube';window.__tubePosition={...sites.tube};window.__portalRequiresEnemies=false}

const target=new THREE.Vector3(),targetLook=new THREE.Vector3();let yaw=.06,pitch=.34,distance=13.4,followDistance=13.4,recenterClock=0,dragging=false,lastX=0,lastY=0;
...
let actionCooldown=0,actionRing=null,actionPoseTimer=0;
...
function loop(){requestAnimationFrame(loop);const dt=Math.min(.033,clock.getDelta());elapsed+=dt;updatePlayer(dt);updateAction(dt);if(bro&&tube?.state==='idle')bro.update(elapsed,dt,0);tube?.update(elapsed,dt);cameraDesired(dt);renderer.render(scene,camera)}
(async()=>{try{sites=buildWorld();worldBounds=new THREE.Box3().setFromObject(world);await createPlayer();createTube();progress('Prewarming tube and shaders…',82);await tube.prewarm();progress('World 1 continuous-land test ready',100);window.__pass='pass9-v1b';window.__world1Ready=true;window.__actionReady=true;window.__rigAnimationLive=true;setTimeout(()=>boot.classList.add('hide'),180);loop()}catch(err){console.error(err);errorEl.textContent=err?.stack||err?.message||String(err);fatal.classList.add('show')}})();