  ...
  function HybridBlob(parts, options) {
    ...
    if (options?.type === 'text/javascript' && parts?.length === 1 && typeof parts[0] === 'string') {
      ...
      if (source.includes('function loadHero(){') && source.includes("heroPortalState='loading'")) {
        ...
        // Trigger only after deliberate entry into the tube footprint.
        ...
        // Correct camera-relative stick geometry. Up = camera forward, right = screen-right.
        // The previous right vector was reversed, making quarter-circle directions feel wrong.
        ...
        if (source.includes(moveBasisNeedle)) {
          source = source.replace(moveBasisNeedle, moveBasisInsert);
          movementPatched = true;
        } else {
          console.error('[Hybrid v0.9 Phase 1.3] analog movement basis target not found');
        }

        // Keep camera heading stable while the thumb is actively steering. Recenter only
        // after release, so a held 10-o'clock direction does not rotate underneath the user.
        ...
        if (source.includes(cameraNeedle)) {
          source = source.replace(cameraNeedle, cameraInsert);
          cameraPatched = true;
        } else {
          console.error('[Hybrid v0.9 Phase 1.3] camera steering target not found');
        }

        // Once physically loaded, begin conversion automatically.
        ...
        // Hide the later v0.8.7 polish group when conversion begins. Those meshes were added
        // after the dissolve shader was wired, so otherwise hair/gear can appear to float.
        ...
        // Restore polish whenever the hero is restored outside.
        ...
        // Finish essentially at disappearance instead of holding on an apparently frozen tube.
        ...
        if (source.includes(phaseDurNeedle)) {
          source = source.replace(phaseDurNeedle, "const phaseDur={charge:2.60,convert:2.65,dissipate:.08,afterglow:.06};");
        }
        ...
        // Restore the authored controller action rig.
        ...
        // Re-stage the controller and BOTH arm pivots so each fist lands beside its own grip.
        ...
        if (source.includes(armsNeedle)) {
          source = source.replace(armsNeedle, armsInsert);
          twoHandPatched = true;
        } else {
          console.error('[Hybrid v0.9 Phase 1.3] two-hand controller pose target not found');
        }

        // Beam origin and direction come from the physical controller emitter.
        ...
        // Finish and respawn on the far high plateau.
        ...
        next = [source];
        if (entryPatched && activationPatched && finishPatched && actionPatched && beamPatched && stallPatched && movementPatched && cameraPatched && twoHandPatched) {
          console.info('[Hybrid v0.9 Phase 1.3] natural movement + two-hand controller + portal lifecycle armed');
        } else {
          console.error('[Hybrid v0.9 Phase 1.3] lifecycle patch incomplete');
        }
      }
    }
