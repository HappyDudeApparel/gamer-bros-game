...
1. **`PRISM_KIT_HANDOFF.md`** — current branch, current toolchain state,
   what's actually built vs. not, the one manual step still open
   (canonical reference images), and the immediate next task.
2. **`PRISM_KIT_RULES.md`** — the rules you must not violate (never build
...
3. **`PRISM_KIT_PLAN.md`** — why the pivot away from the Nature Kit
...
This file, `PRISM_KIT_HANDOFF.md`, `PRISM_KIT_RULES.md`, and
`PRISM_KIT_PLAN.md` are the authoritative state. If anything in an old
chat transcript or a code comment contradicts them, these files win.
